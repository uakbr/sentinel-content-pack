...
if ! az deployment group create \
  ...
  --output none; then
    echo -e "${RED}Failed to deploy API connections${NC}"
    exit 1
fi
...
if compgen -G "playbooks/*.json" > /dev/null; then
...
if compgen -G "watchlists/*.csv" > /dev/null; then
    ...
    echo -e "${YELLOW}  No watchlists found${NC}"
fi
